...
(function ($) {
    ...
    $.fn.ComboBoxClearItems = function () {
        return $(this).comboBoxClearItems();
    }
    $.fn.ComboBoxSetValue = function (value, isIgnoreCase) {
        return $(this).comboBoxSetValue(value, isIgnoreCase);
    }
    ...
    $.fn.ComboBoxTreeClearItems = function () {
        return $(this).comboBoxTreeClearItems();
    }
    ...
    $(".ui-filter-text").click(function () {
        ...
        $(document).click(function (e) {
            if (!$('.ui-filter-list').is(":hidden")) {
                ...
                if ($(tar).closest('.ui-select-option').length == 0 && $(tar).closest('.ui-filter-list').length == 0 && $(tar).closest('.ui-filter-text').length == 0) {
                    $(".ui-filter-text").css('border-bottom-color', '#ccc');
                    $(".ui-filter-list").slideUp(10);
                    $(tar).parents('.ui-filter-list').removeClass("active")
                }
            }
        });
        //added by Cathy 2017-07-20
